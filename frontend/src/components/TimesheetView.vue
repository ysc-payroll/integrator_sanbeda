  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button
          v-if="filterStatus === 'error'"
          @click="exportMismatchEmployees"
          class="btn btn-secondary"
        >
          <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          Export Mismatch Employee ID
        </button>
...
const retryAllErrors = async () => {
  try {
    const result = await bridgeService.retryAllFailedTimesheets(
    ...
    error(err.message || 'Failed to retry errors')
  }
}

const exportMismatchEmployees = async () => {
  try {
    const result = await bridgeService.exportMismatchEmployees(
      filterDateFrom.value,
      filterDateTo.value
    )
    success(result.message)
